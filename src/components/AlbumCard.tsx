...
export default function AlbumCard({ album, onClick }: Props) {
  return (
    <button
      onClick={onClick}
      className="group flex flex-col gap-2 text-left hover:bg-white/5 rounded-xl p-3 transition-colors"
    >
      ...
      <div className="min-w-0">
        ...
        {album.year && <p className="text-white/30 text-xs">{album.year}</p>}
      </div>
    </button>
  );
}
