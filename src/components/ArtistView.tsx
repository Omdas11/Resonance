      ...
      {demoArtists.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {demoArtists.map(artist => (
            ...
            >
              <div className="w-24 h-24 rounded-full overflow-hidden bg-[#3a3a3c] relative">
                {artist.image
              ...
              <div className="text-center">
                ...
                <p className="text-white/40 text-xs">{artist.albums.length} album{artist.albums.length !== 1 ? 's' : ''}</p>
              </div>
            </button>
          ))}
        </div>
